...
**What does `APPENDONLY=TRUE` actually do under the hood?**
...
* Enable Compression & Columnar Storage: It opens up options for heavy compression algorithms (e.g., `zstd, zlib`) and columnar layouts (`orientation=column`), drastically reducing S3/disk footprint.  
...
* Strict Target & Parameter Locking All backups within an incremental chain must share identical configuration options (e.g.`, --leaf-partition-data, --plugin-config`) and reside on the exact same storage target/S3 bucket. You cannot mix storage targets mid-chain.  
...
| Storage target sizing | Dataset size × retention × change rate, less any target-side dedup/compression. |
...
* Data is Partitioned and AO-Heavy: Historical partitions (e.g., past months/years) are AO and never modified; only the active daily partition receives new INSERTs.  
* Strict RPO Requirements: You need frequent recovery points (e.g., every 4 hours) where taking full backups is physically impossible due to time constraints.
...
The operational standard: every backup monitored on return code and history status, failures alerting automatically, and the existence of a current restorable set verified rather than presumed. Periodic **test restores onto a recovery cluster** are the only proof the DR path actually works.
